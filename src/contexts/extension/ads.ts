...
export default class AdBlocker {
    ...
    private static async tick() {
        ...
        if (
            this.autoStop &&
            this.totalAdsRemoveLastCoupleTicks.every((x) => x !== undefined)
        ) {
            const ms =
                this.REMOVE_CHECK_INTERVAL *
                this.totalAdsRemoveLastCoupleTicks.length;
            const seconds = ms / 1000;
            logger.debug(
                `AdBlocker stopped no more ads removed in the last ${seconds} seconds.`
            );
            this.stop();
        }
    }
    ...
    public static async removePrivacyPopup() {
        ...
        await Promise.waitFor(
            (resolve) => $("#onetrust-consent-sdk").remove().length && resolve()
        ).catch((_) => logger.warn("Privacy popup not visible."));
    }
}
